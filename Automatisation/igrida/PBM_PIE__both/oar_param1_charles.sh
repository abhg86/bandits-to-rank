#!/bin/bash
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# version de test Charles
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

#OAR -l core=1,walltime=02:05:00
#OAR --array-param-file ./python_param1_charles.txt
#OAR --name=PBM_PIE__both
#OAR -O /temp_dd/igrida-fs1/cgauthie/SCRATCH/outputs/charles_PBM_PIE__both.%jobid%.output
#OAR -E /temp_dd/igrida-fs1/cgauthie/SCRATCH/outputs/charles_PBM_PIE__both.%jobid%.error

EXECUTABLE="python3 $HOME/src/bandits-to-rank/Automatisation/igrida/exp.py"
export SCRATCHDIR="/temp_dd/igrida-fs1/cgauthie/SCRATCH"
