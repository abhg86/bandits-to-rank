...
nb_trial = 10000000
nb_check_points = 100
env_names = ["--Yandex %d" % (i) for i in range(10)]+["--Yandex_CM %d" % (i) for i in range(10)] # ["--test_CM"]+["--test"]+["--Yandex_CM %d" % (i) for i in range(10)] +["--std", "--xxsmall", "--big"] + ["--KDD %d" % (i) for i in range(8)] + ["--Yandex %d" % (i) for i in range(10)]
...
if __name__ == "__main__":
    ...
    with open("python_param_Yandex_CM.txt", "w") as file:
        for start_game, env_name, ranking, relevant_pos in product(start_games, env_names, PositionsRankings,relevant_positions):
            ...
            args_player = f'--UniRank {output_path}{" --force" if force else ""} --nb_checkpoints {nb_check_points}'
