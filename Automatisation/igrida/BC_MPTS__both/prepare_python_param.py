...
# 1 game = 1h10   =>   10 games per node (to target 12 hours)
total_nb_game = 20
nb_game_per_node = 3
...
if __name__ == "__main__":
    ...
    with open("python_param.txt", "w") as file:
        for start_game, oracle, env_name in product(start_games, [True, False], env_names):
            args = f'--play {min(total_nb_game - start_game, nb_game_per_node)} -s {start_game} {nb_trial} -r {record_length} {env_name} --BC-MPTS{" --oracle" if oracle else ""} {output_path}{" --force" if force else ""}'
