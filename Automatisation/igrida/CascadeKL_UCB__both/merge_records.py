...
env_names =['--Yandex_all']#["--KDD_all"]#['--test_CM']+["--test"]+ ['--Yandex_CM_all'] +["--std", "--xxsmall", "--big"] + ["--KDD_all"] + ['--Yandex_all']
...
if __name__ == "__main__":
    for env_name in env_names:
        ...
        for ranking, nb_trial,real_pos, nb_position,nb_item in product(PositionsRankings, nb_trials,relevant_positions,nb_positions,nb_items):
            arg_env = f'--merge {nb_trial} -r {record_length} -l {real_pos} {ranking} {env_name} {nb_position} {nb_item} '
            arg_player = f'--CascadeKL-UCB {input_path}  {output_path}'
