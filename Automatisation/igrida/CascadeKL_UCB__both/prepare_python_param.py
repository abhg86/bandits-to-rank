...
if __name__ == "__main__":
    ...
    with open("python_param_Yandex_KDD.txt", "w") as file:
        for start_game, env_name, ranking, OSUB_type in product(start_games, env_names, PositionsRankings):
            args_env = f'--play {min(total_nb_game - start_game, nb_game_per_node)} -s {start_game} {nb_trial} -r {record_length} {ranking} {env_name} '
            args_player = f'--CascadeKL-UCB {output_path}{" --force" if force else ""} --nb_checkpoints {nb_check_points}'
