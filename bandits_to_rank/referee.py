...
from bandits_to_rank.kendall_distance import kendall_distance
import time
import numpy as np
...
class Referee:
    ...
    def prepare_new_game(self, query=None):
        ...
        self.running_cumulative_leader_reward = 0
        self.running_kendall_distance = []
        ...
        self.running_leader_reward_recorded = []
        self.running_kendall_distance_recorded = []
    ...
    def play_game(self, player, query=None, new_game=True, nb_trials_before_break=None, nb_relevant_positions=None):
        ...
        if nb_relevant_positions is None :
            nb_relevant_positions = self.env.get_setting()[1]
        if query is None:
            self.best_expected_reward_vector =self.env.get_expected_reward(self.env.get_best_index())
            self.indice_relevant_pos = self.best_expected_reward_vector.argsort()[-nb_relevant_positions:][::-1]
            self.running_expected_best_reward = sum(self.best_expected_reward_vector[self.indice_relevant_pos])
            print( self.best_expected_reward_vector)
            print(self.indice_relevant_pos)
            print(self.running_expected_best_reward)
            #exit(0)
        else:
            self.best_expected_reward_vector = self.env.get_expected_reward(self.env.get_best_index(query), query)
            self.indice_relevant_pos = self.best_expected_reward_vector.argsort()[-nb_relevant_positions:][::-1]
            self.running_expected_best_reward = sum(self.best_expected_reward_vector[self.indice_relevant_pos])
        ...
        for self.running_t in range(start, end):
            ...
            try :
                leader = player.get_leader()
                leader_reward = self.env.get_reward(leader)
                self.running_cumulative_leader_reward += sum(leader_reward[self.indice_relevant_pos])
                self.running_kendall_distance.append(kendall_distance(leader, self.env.get_best_index()))
                if self.running_t in self.time_record_expected:
                    self.running_leader_reward_recorded.append(self.running_cumulative_leader_reward)
                    self.running_kendall_distance_recorded += self.running_kendall_distance
            except:
                pass

            self.running_cumulative_reward += sum(rewards[self.indice_relevant_pos])
            self.running_cumulative_expected_reward += sum(expected_reward[self.indice_relevant_pos])
            self.running_cumulative_reject_time += reject

            player.update(propositions, rewards)
            if isinstance(self.env, Environment_PBM):
                self.update_matrix(propositions[self.indice_relevant_pos], rewards[self.indice_relevant_pos])
            ...
            if self.running_t % (self.nb_trials / self.print_trial) == 0:
                ...
                try:
                    if False:
                        ...
                        print(player.leader, round(self.env.get_expected_reward(np.array(player.leader)).sum(),3),
                              round(self.running_expected_best_reward-self.env.get_expected_reward(np.array(player.leader)).sum(),3),
                              propositions, round(self.env.get_expected_reward(propositions).sum(),3),
                              round(self.running_expected_best_reward-sum(expected_reward[self.indice_relevant_pos]),3)
                              )
                except:
                    pass
                try:
                    if True:
                        print(len(player.logs))
                        print(player.leader)
                        print(player.logs[player.leader])
                        print(player.leader, round(self.env.get_expected_reward(np.array(player.leader)).sum(),3),
                              round(self.running_expected_best_reward-self.env.get_expected_reward(np.array(player.leader)).sum(),3),
                              propositions, round(self.env.get_expected_reward(propositions).sum(),3),
                              round(self.running_expected_best_reward-sum(expected_reward[self.indice_relevant_pos]),3)
                ...
                try:
                    if True:
                        print(len(player.node_logs), len(player.shared_logs))
                        print(player.leader)
                        print(player.node_logs[player.leader])
                        local_rho_hat = np.zeros(player.nb_positions)
                        local_trials = np.zeros(player.nb_positions)
                        for k in range(player.nb_positions):
                            local_rho_hat[k] = player.shared_logs[tuple(np.sort(player.leader[:k]))]['hat_rhos'][player.leader[k]]
                            local_trials[k] = player.shared_logs[tuple(np.sort(player.leader[:k]))]['nb_trials'][player.leader[k]]
                        print(local_rho_hat, local_trials)
                        print(player.leader, round(self.env.get_expected_reward(np.array(player.leader)).sum(),3),
                              round(self.running_expected_best_reward-self.env.get_expected_reward(np.array(player.leader)).sum(),3),
                              propositions, round(self.env.get_expected_reward(propositions).sum(),3),
                              round(self.running_expected_best_reward-sum(expected_reward[self.indice_relevant_pos]),3)
                ...
                try:
                    if False:
                        print("leader :", player.get_leader())
                        print("player.n : ", player.n)
                        print("player.s : ", player.s)
                except:
                    pass
            ...
            if self.running_t == self.nb_trials - 1:
                ...
                try :
                    self.record_results['leader_reward'].append(self.running_leader_reward_recorded)
                    self.record_results['kendall_distance'].append(self.running_kendall_distance_recorded)
                except:
                    pass
                if query is None:
        ...
        return self.running_expected_best_reward - sum(expected_reward[self.indice_relevant_pos])     # instantaneous regret
    ...
    def get_regret_leader(self):
        sous = np.array(self.record_results['expected_best_reward']) - np.array(self.record_results['leader_reward'])
        reg = np.mean(sous, axis=0)
        return reg
    
    def get_kendall_distance(self):
        return np.mean(self.record_results['kendall_distance'], axis=0)
    ...
    def show(self):
        ...
        try :
            plt.plot(trials, np.cumsum(self.get_regret_leader(), axis=0)[trials], label='leader')
            plt.xlabel('Time')
            plt.ylabel('Cumulative Leader Regret')
            plt.legend()
            plt.grid(True)
            plt.loglog()
            plt.show()

            plt.plot(trials, np.cumsum(self.get_kendall_distance(), axis=0)[trials], label='leader')
            plt.xlabel('Time')
            plt.ylabel('Cumulative Leader Regret')
            plt.legend()
            plt.grid(True)
