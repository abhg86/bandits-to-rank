...
class Target :
    ...
    def pdf_(self, part_prev):
        #### Renvoie la proba d'un theta ( ou kappa)
        res = 1
        if self.on_theta:
    ...
    def pdf_multiparticule(self, part_prev_list): 
        #### Renvoie la liste des x (= theta / kappa) et des proba des proba associé pour une liste de particule donnée
        x = []
        y = []
        if self.on_theta: 
    ...
    # def pdf_thetas(self, part_prev):
    #     #### Renvoie les proba de tous les thetas ( ou kappas)
    #     if self.on_theta:
    #         # !!! XXXX il faut mettre à zéro les proba quand theta trop petit ou trop grand !!!!
    #         res = np.ones(part_prev[0].shape)
    #         for l in range(kappa.shape[0]):
    #             #print ('len_kappa',kappa.shape[0])
    #             #print('l',l)
    #             #print ('succ',self.succ)
    #             kappa_mono = kappa[l]
    #             succ_l = self.succ[l]
    #             fail_l = self.fail[l]
    #             res *= st.beta(succ_l+1, fail_l+1).pdf(theta*kappa_mono) * kappa_mono
    #         return res
    #     else:
    #         res = np.ones(kappa.shape)
    #         for l in range(theta.shape[0]):
    #             theta_mono = theta[l]
    #             succ_l = self.succ[l]
    #             fail_l = self.fail[l]
    #             res *= st.beta(succ_l+1, fail_l+1).pdf(theta_mono*kappa) * theta_mono
    #         return res
