...
class Proposal_Uniform:
    """
    """

    def __init__(self, a=0., b=1., k=0, on_theta=True):
        self.a = a
        self.b = b
        if on_theta:
            self.i_param0 = 0
    ...
    def next_part(self, part_prev):
        ...
        part[self.i_param0][self.i_param1] = np.random.uniform(self.a, self.b)
        return part
...
class Proposal_RW_logit:
    ...
    def next_part(self, part_prev):
        part = deepcopy(part_prev)
        prop = np.random.normal(logit(part[self.i_param0][self.i_param1]), self.sigma)
        part[self.i_param0][self.i_param1] = expit(prop)
        return part
