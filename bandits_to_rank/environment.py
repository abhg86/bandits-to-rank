...
class Environment_multirequest_PBM:
    ...
    def shuffle(self, positions_ranking=PositionsRanking.FIXED):
        ...
        >>> from bandits_to_rank.environment import Environment_multirequest_PBM, PositionsRanking
        >>> import random
        >>> import numpy as np
        >>> np.set_printoptions(precision=2)
...
class Environment_Cascade:
    ...
    def __init__(self, thetas, order_view, label=None):
        ...
        if np.any(order_view[:-1] > order_view[1:]):
            raise ValueError('CM model correctly defined only when positions are viewed from the first to the last')
    ...
    def shuffle(self, positions_ranking=PositionsRanking.FIXED):
        ...
        >>> from bandits_to_rank.environment import Environment_Cascade, PositionsRanking
        >>> import random
        >>> import numpy as np
        >>> np.set_printoptions(precision=2)
    ...
    def get_best_index(self):
        # TODO: To look at, handling of diorder kappa is handled in an incoherent way.
        # TODO: check if PBM is fine or not
