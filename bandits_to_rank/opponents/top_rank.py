...
class TOP_RANK:
    ...
    def __init__(self, nb_arms, T, horizon_time_known=True,doubling_trick_active=False, nb_positions=None, discount_factor=None, lag=1, prior_s=1, prior_f=1):
        ...
        self.time_horizon_known = horizon_time_known
        self.doubling_trick_active = doubling_trick_active
        if horizon_time_known:
            self.initial_time_horizon = T
        else:
            self.initial_time_horizon = None
            self.delta = T
    ...
    def clean(self):
        ...
        # clean the log
        self.time = 0
        self.time_horizon = self.initial_time_horizon
        if self.time_horizon_known:
            self.delta = self.delta_from_horizon(self.time_horizon)
    ...
    def update(self, propositions, rewards):
        self.time += 1
        if self.doubling_trick_active and self.time >= self.time_horizon:   #double-tricking
            print(f'doubling trick at T={self.time}')
            self.time_horizon *= 2
            self.delta = self.delta_from_horizon(self.time_horizon)
            self.clean_at_doubling_trick()
