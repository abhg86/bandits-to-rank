...
class PMED:
    ...
    def update_thetas_hat_kappas_hat(self, num_steps=100, plot=False, verbose=False, epsilon=10**-14):
        ...
        if plot:
            ...
            plt.ylabel("thetas[0]")
            for i in range(4):
                plt.plot(trace['theta'][:, i], ':', color=f'C{i+1}', label=f'$\\theta_{i}$')
            plt.grid()
            plt.ylim([-0.05, 1.05])
            plt.legend()
            ...
            plt.ylabel("kappas[0]")
            for i in range(4):
                plt.plot(trace['kappa'][:, i], ':', color=f'C{i+1}', label=f'$\\kappa_{i}$')
            plt.grid()
            plt.ylim([-0.05, 1.05])
            plt.legend()
    ...
    def optimize_constraint(self, qo, num_steps=10000, plot=True, verbose=False, epsilon=10**-14):
        if verbose:
            print('theta_hat:', self.thetas_hat)
            print('kappas_hat:', self.kappas_hat)
