...
for c in [1., 10., 100., 1000., 10000., 100000., 1000000.]:
    refs[c] = retrieve_data_from_zip(resdir + 'KDD_all__Bandit_EGreedy_SVD_' + str(c) + '_c_1_maj__games_10000000_nb_trials_1000_record_length_160_games.gz')
...
refs[f'TopRank, semi-oracle'] = retrieve_data_from_zip(resdir + f'KDD_all__extended_kappas__Bandit_TopRank_oracle__games_10000000_nb_trials_1000_record_length_160_games.gz')
refs[f'TopRank, sorted'] = retrieve_data_from_zip(resdir + f'KDD_all__extended_kappas__sorted_kappa__Bandit_TopRank_oracle__games_10000000_nb_trials_1000_record_length_160_games.gz')
...
#    plt.xlim([1000, 20000000])
#    plt.ylim([70,200000])
plt.savefig("./result/graph/KDD__BubbleRank_TopRank_20g_10000000t.pdf", bbox_inches = 'tight',
    pad_inches = 0)
...
for c in cs:
    ...
    for nb_step, random_start in product(nb_steps, random_starts):
        ...
        #myplot(ref=refs[label], label=label, color=f'C{i % 10}', linestyle=line_styles[j])
        j+=1
    i += 1
...
for delta in [1e-28]:
    refs[f'BubbleRank, {delta}, semi-oracle'] = retrieve_data_from_zip(resdir + f'KDD_all__extended_kappas__Bandit_BubbleRank_{delta}_delta_oracle__games_10000000_nb_trials_1000_record_length_160_games.gz')
    refs[f'BubbleRank, {delta}, sorted'] = retrieve_data_from_zip(resdir + f'KDD_all__extended_kappas__sorted_kappa__Bandit_BubbleRank_{delta}_delta_oracle__games_10000000_nb_trials_1000_record_length_160_games.gz')
refs[f'TopRank, semi-oracle'] = retrieve_data_from_zip(resdir + f'KDD_all__extended_kappas__Bandit_TopRank_oracle__games_10000000_nb_trials_1000_record_length_160_games.gz')
refs[f'TopRank, sorted'] = retrieve_data_from_zip(resdir + f'KDD_all__extended_kappas__sorted_kappa__Bandit_TopRank_oracle__games_10000000_nb_trials_1000_record_length_160_games.gz')
