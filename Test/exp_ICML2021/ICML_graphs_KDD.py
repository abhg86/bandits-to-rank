...
# %%
resdir = "exp_ICML2021/results/real_KDD/"

refs={}


refs['UniPBrank'] = retrieve_data_from_zip(resdir +'KDD_all__shuffled_kappa__Bandit_UniPBRank_10000000_T__games_10000000_nb_trials_1000_record_length_160_games.gz')


# %% jupyter={"outputs_hidden": true}
resdir = "exp_IDA/results/real_KDD/"

refs['TopRank'] = {}

refs['TopRank']['PBM_decrease'] = retrieve_data_from_zip(resdir +'KDD_all__sorted_kappa__extended_kappas__Bandit_TopRank_1000000.0_delta_TimeHorizonKnown___games_1000000_nb_trials_1000_record_length_160_games.gz')

refs['KL-MLMR'] = retrieve_data_from_zip(resdir + 'KDD_all__sorted_kappa__Bandit_KL-MLMR_10000000_horizon__games_10000000_nb_trials_1000_record_length_160_games.gz')
...
#for c in [10000.]:
    #refs['PBM_decrease'][f'eGreedy_c_{c}'] = retrieve_data_from_zip(resdir + f'KDD_all__Bandit_EGreedy_SVD_{c}_c_1_maj__games_100000_nb_trials_1000_record_length_160_games.gz')
...
refs[f'PMED']={}
refs[f'PMED']['PBM_decrease'] = retrieve_data_from_zip(resdir + f'KDD_all__sorted_kappa__Bandit_PMED_1.0_alpha_1_gap_MLE_0_gap_q__games_100000_nb_trials_1000_record_length_133_games.gz')


for delta in [0.1]:
    refs[f'TopRank_delta_{delta}'] = retrieve_data_from_zip(resdir + f'KDD_all__extended_kappas__sorted_kappa__Bandit_TopRank_oracle_{delta}_delta____games_100000_nb_trials_1000_record_length_160_games.gz')


# %%
for rec in refs[f'PMED']['PBM_decrease'].record_results['env_parameters'][:10]:
...
myplot(ref = refs['eGreedy_c_10000.0'], label='$\epsilon_n$-greedy, c=$10^4$', color = 'C1', linestyle = '-')

myplot(ref = refs['PB-MHB, proposal=TGRW, c=1000.0, m=1'], label='PB_MHB, c=$10^3$, m=1', color = 'C4', linestyle = '-')
myplot(ref = refs['UniPBrank'], label='GRAB', color = 'C3', linestyle = '-')

#myplot(ref = refs['KL-MLMR'], label='MLMR', color = 'C4', linestyle = '-')

myplot(ref = refs['TopRank']['PBM_decrease'], label='TopRank', color = 'C2', linestyle = '-')
myplot(ref = refs['TopRank_delta_0.1'], label='TopRank $delta$=0.1', color = 'C2', linestyle = '--')

myplot(ref = refs['PMED']['PBM_decrease'], label='PMED', color = 'C5', linestyle = '--')
...
plt.ylim([1, 200000])
plt.savefig("exp_ICML2021/results/graph/KDD_PBM_decrease.pdf", bbox_inches = 'tight',
    pad_inches = 0)
